...
pdf.add_page()
h1('2026 年中 · 完整持仓分析报告')
note('日期：2026-07-01   |   口径：市值已脱敏、按固定单位换算   |   总仓位约 31.6 份')
...
pdf.add_page()
h2('三、领域大类占比')
table(['大类','份额','占比','含标的','评价'],
 [['加密/区块链','6.0','19.0%','BTC·CRCL','熊市失血，需减'],
  ['AI存储/内存','5.5','17.4%','海力士·DRAM(ETF)·MU','HBM 超级周期，最强主线'],
  ['AI算力云','4.7','14.9%','NBIS·CRWV','半年最大赢家，核心持有'],
  ['新能源车','4.0','12.7%','XPEV','单票过重，需砍'],
  ['AI算力/芯片','3.7','11.7%','AMD·MRVL·COHR','赢家阵营，方向正确'],
  ['互联网/科技','2.6','8.2%','GOOG·NOK','稳健底仓'],
  ['AI半导体设备','2.4','7.6%','TSM·AMAT','代工/设备龙头'],
  ['前沿题材','1.2','3.8%','QUBT·XE','消耗仓，可清理'],
  ['现金','1.0','3.2%','现金','弹药偏少'],
  ['消费','0.5','1.6%','泡泡玛特','非核心']],
 [22,10,12,42,32])
...
for p in [
 ...
 '1. XPEV 是全组合最大单票（4 份 / 12.7%），却是持续下跌的失血标的（YTD -35%），与"早就计划退出新能源车"的判断严重冲突——又一次温水煮青蛙。',
 '2. 加密敞口高达 19%（BTC 3 + CRCL 3），而 BTC 已跌破熊市生命线（半年 -50%）、CRCL 逻辑崩塌（护城河被 OpenAI USD 冲击），双重承压。',
 '3. 真正的赢家（NBIS、MU、AMD、AMAT）反而仓位偏小或分散，MU 仅 0.5 份，没能充分放大主线收益。',
 '4. 存储敞口存在隐性重叠：DRAM(ETF) 3 份内部已含 MU/海力士，叠加单独持有的 MU 0.5 + 海力士 2，实际存储集中度高于表面。',
