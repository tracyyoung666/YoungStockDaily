...
**五档信号判定规则（按优先级从上到下，命中即定，不再往下判）：**

```
Strong Sell▼▼（极端超买/破顶，重仓止盈）—— 命中任一：
   1. RSI6 > 92
   2. RSI6 > 85 且 贴近52周新高（dist_from_52w_high_pct > -2%）
   3. RSI6 > 85 且 单日/盘前盘后涨幅 > 10%
   → color:#15803d（深绿）

定投Sell▼（常规止盈减仓）—— 命中任一：
   1. RSI6 > 85
   2. dist_from_52w_high_pct > -2% 且 RSI6 > 70
   3. 单日/盘前盘后涨幅 > 10% 且 RSI6 > 75
   → color:#16a34a（绿）

Strong Buy▲▲（极端超卖/深度错杀，加大力度抄底）—— 命中任一：
   1. RSI6 < 12
   2. RSI6 < 20 且 回撤 < -30%
   3. RSI6 < 20 且 单日/盘前盘后跌幅 > 6%
   4. 回撤 < -40% 且 RSI6 < 30
   → color:#b91c1c（深红）

定投Buy▲（常规分批买入）—— 命中任一：
   1. RSI6 < 20
   2. dist_from_52w_high_pct < -20% 且 RSI6 < 45
   3. 单日/盘前盘后跌幅 > 6% 且 RSI6 < 40
   → color:#dc2626（红）
