...
<ul>
  ...
  <li><strong>🎯 定投信号：</strong><span style="color:#b91c1c">Strong Buy▲▲</span> → <span style="color:#b91c1c">Strong Buy（🟢技术错杀型）</span> — RSI6=13.2超卖 + 距52W高-24% + 单日-6.2%命中规则；净利率9.7%、机构卖出0家、下跌系板块共振，0项Red Flag，可1.5倍加倍定投</li>
  <li><strong>建议：</strong>...</li>
</ul>
<!-- 重复 15 只。🎯 定投信号 = 由"定投买卖点判定规则"（见步骤 2.55）产出，五档取值：
     Strong Buy▲▲ / 定投Buy▲ / 观望— / 定投Sell▼ / Strong Sell▼▼；文案必须写明触发的量化依据
     ⚠️ 买入侧（Strong Buy/Buy）必须经过"二次基本面校验"，写成
     "<技术档位> → <校验后可执行档位>（<🟢技术错杀/🟡存疑/🔴基本面恶化>）— <依据>" -->

<h3>🎯 定投买卖点建议（全量 15 只）</h3>
<!-- 每次必出，覆盖全部自选股；信号规则见步骤 2.55 -->
<table class="stock-table">
<thead><tr>
  <th>代码</th><th>名称</th><th>收盘价</th><th>RSI6</th><th>距52W高</th><th>定投信号</th><th>基本面校验</th><th>依据 / 操作建议</th>
</tr></thead>
<tbody>
<tr>
  <td><strong>MU</strong></td><td>美光科技</td><td>$518.46</td><td>13.2</td>
  <td>-24.0%</td>
  <td><span style="color:#b91c1c">Strong Buy▲▲</span></td>
  <td>🟢技术错杀</td>
  <td>RSI6超卖+深度回调，净利率9.7%/机构卖出0家，1.5倍加倍抄底</td>
</tr>
<!-- 15 只全部列出。定投信号五档：
     Strong Buy▲▲(#b91c1c) / 定投Buy▲(#dc2626) / 观望—(#6b7280) / 定投Sell▼(#16a34a) / Strong Sell▼▼(#15803d)
     基本面校验列取值：🟢技术错杀 / 🟡存疑 / 🔴基本面恶化 / —（Sell侧与观望档不触发校验） -->
</tbody>
</table>
...
**六档信号与建议份额（择时分级下注 · 2026-07-29 重构）**

> 用户明确要求：**不做传统定投，要择时**。故"定投"与"Strong Buy"不再是强弱关系，而是**两种不同性质的信号**——机械扣款 vs 右侧确认下注。
> 1 份 = 单次计划投入的基准金额（相对单位）。

| 档位 | 性质 | 份额 | 颜色 |
|---|---|---|---|
| `Strong Buy▲▲` | 择时·右侧确认 | **+2.0** | `#b91c1c` |
| `定投Buy▲` | 择时·左侧试探 | **+1.0**（空头排列砍半 0.5） | `#dc2626` |
| `定投▲` | 传统定投·机械扣款 | **+0.5** | `#f59e0b` |
| `观望—` | 不动作 | 0 | `#6b7280` |
| `定投Sell▼` | 减仓 | **-1.0** | `#16a34a` |
| `Strong Sell▼▼` | 重仓止盈 | **-2.0** | `#15803d` |

**核心机制一：动态阈值（用户要求必须落在 5%~10%）**
```
σ = 近20日日收益率标准差（来自 performance.json）
反弹确认阈值 R = clamp(1.5 × σ, 5%, 10%)   买入侧（更严）
回调确认阈值 D = clamp(1.2 × σ, 5%, 10%)   卖出侧（更敏捷，防利润回吐）
```
⚠️ 系数 k=1.5 是**实测最优**（11/15 落在 5~10% 区间内）。k=2.5 会让 11/15 全部撞上限、退化为"所有标的统一 10%"，标准化失去意义；k=1.0 则只有 4/15 在区间内。买卖用不同系数体现不对称原则：**止盈要比抄底更敏捷**。

**核心机制二：趋势状态前置判定**
```
空头排列  MA5<MA10<MA20 且 收盘<MA20   → bear（危险区，所有超卖信号降级）
多头排列  MA5>MA10>MA20 且 收盘>MA20   → bull
震荡      其余                          → range
```

**买入侧规则**
```
Strong Buy▲▲（+2.0）· 必须同时满足全部 4 条
  1. 深跌前提：距52周高 < -20%
  2. 曾经超卖：近5日内出现过 RSI6 < 25
  3. 反弹确认：自20日低点反弹幅度 ≥ R
  4. 结构修复：收盘价 ≥ MA10        ← 防"一根情绪阳线"假确认
  ⚠️ 若20日低点出现在最近1个交易日内 → 反弹尚未形成，一律不给

定投Buy▲（+1.0）· 满足任一且未达 Strong Buy
  1. RSI6 < 20
  2. 距52周高 < -20% 且 RSI6 < 45
  3. 当下跌幅 > 6% 且 RSI6 < 40
  · 空头排列 → 份额砍半至 +0.5，标注"逆势试探"

定投▲（+0.5）· 无任何技术信号时的机械定投，保留参与度

🚫 一票否决（不得发 Strong Buy）
  · 空头排列 + 当日跌幅>6%（暴跌中接飞刀）
  · 基本面 ≥2 项 Red Flag → 最高 Buy 且份额压至 0.3
  · 20日序列缺失/不足6点/严重滞后(>5天) → 最高 Buy 并标注
```
🚨 **修掉的逻辑倒置**：原规则"RSI6<20 且当日跌幅>6% → Strong Buy"是典型接飞刀，现降为 Buy，空头排列下更直接禁止 Strong Buy。

**卖出侧规则（把铁律一写进规则）**
```
【前置】刚冲高判定 is_fresh_surge()：近10日累涨>15%
        且 高点在最近3日内 且 高点后未连跌≥3天
        ⚠️ 空头排列下禁止一切冲高止盈（下跌趋势中不存在"保护刚到顶利润"）

Strong Sell▼▼（-2.0）· 同时满足
  1. 刚冲高  2. 自20日高回调 ≥ D  3. 收盘 < MA10（跌破趋势）

定投Sell▼（-1.0）· 满足任一
  1. 刚冲高 且 回调 ≥ 5%        ← 铁律一原文直译
  2. RSI6 > 85 且 收盘 < MA5

⚠️ 高位预警（不减仓，仅提示）
  RSI6>85 或 距52周高>-2% 但尚未回调 → 标注「⚠️高位·不宜新买入」
  专门解决"超买≠该卖"的语义混淆：空仓者与满仓者需要不同动作
```

**空头排列连续定投熔断**：空头排列下连续机械定投超过 4 次 → 降为观望，等趋势转好。计数持久化于 `portfolio/dca_state.json`，同一天多次运行只计一次，趋势/信号改善时自动重置。

**止损锚点（对应铁律三）**：每笔买入信号必须输出 `stop_loss_anchor = 近20日最低收盘`，报告中写明"跌破 $X 即视为判断失败"，让小仓位止损有据可依。

**执行脚本**：`scripts/compute_signals.py`，产出 `_signals.json`（含 advice_key/shares/trend/σ/阈值/反弹回调幅度/均线/止损锚点/reason）。渲染与 HTML 生成必须消费该文件，**严禁另写一套判定逻辑**。


**🚨 RSI 缺失时的护栏（重要教训 2026-07-28）**：降级为纯价格判定时，**严禁仅凭"距52周高点跌得深"就发 Strong Buy**——趋势崩坏的标的往往跌得最深（如 MSTR -76%、QUBT -69%），深跌不等于抄底良机。纯价格判定规则：
```
Strong Buy 必须同时满足：回撤 < -40% 且 当下跌幅 > 6%（深度回撤 + 恐慌下跌双重确认）
定投Buy：当下跌幅 > 6%，或（回撤 < -20% 且 当下在跌）
Strong Sell：贴近52周新高 且 涨幅 > 6%
定投Sell：贴近52周新高
其余：观望
```
且文案必须标注"RSI缺失，仅价格判定"。

### 🔬 Strong Buy 二次基本面校验（强制，2026-07-28 确立）

**背景**：2026-07-28 晚报同时出现 4 只 Strong Buy（TSLA/MRVL/CRWV/NOK），但人工复核发现质地天差地别——MRVL 毛利率 50%/净利率 9.7%/机构卖出 0 家（真错杀），而 CRWV 单季净亏 7.4 亿/净利率 -35.6%、NOK 净利同比 -68%（公司自身在恶化）。**技术规则只能识别"跌得够多"，无法识别"公司变坏"。** 若不加校验，会把基本面崩塌的标的和被误伤的优质股放进同一档加倍抄底，风险极高。

**触发条件**：任一股票被机械规则判为 `Strong Buy▲▲` 或 `定投Buy▲` 时，**必须**执行本校验后才能写入报告。Sell 侧与观望档不需要。

**校验输入**（`westock <code> finance` + `rating` + 行情字段，均为现成数据源）：
- 盈利能力：`NetMargin_Q`（净利率）、`GrossMargin_Q`（毛利率）、`NetIncome_Q`（净利）、`eps_ttm`
- 盈利趋势：财报新闻中的净利/营收同比（如"净利润同比减少 68%"）
- 机构共识：`rating` 的买入家数占比、**卖出家数**、覆盖机构总数
- 趋势动能：`chg_20d` / `chg_60d`（中期跌幅，判断是情绪杀还是趋势性下跌）
- 估值：`pe_ratio`（负值=亏损）

**四条否决项（Red Flag，命中任一即不得维持 Strong Buy）：**
```
① 仍在亏损：NetIncome_Q < 0 或 pe_ratio < 0
② 盈利崩塌：最近财报净利同比 < -50%
③ 盈利能力孱弱：NetMargin_Q < 2%（几乎不赚钱，估值无盈利支撑）
④ 趋势性下跌：chg_60d < -30% 且 chg_20d < -20%（非单日情绪杀，是持续趋势）
   ⚠️ 机构评级缺失/覆盖不足 视为"信息不足"，等同一个 Red Flag
```

**分型与降级动作（按 Red Flag 数量为主轴，严禁跳档）：**
```
🟢 技术错杀型（Strong Buy 成立，可加倍）
   条件：0 个 Red Flag，且（净利率 > 5% 或 机构卖出 = 0 家）
   且下跌可归因于板块共振/大盘情绪，而非公司自身利空
   → 维持 Strong Buy▲▲，建议 1.5 倍额度，文案标注「技术错杀型」

🟡 存疑型（降级为常规 Buy）
   条件：命中 1 个 Red Flag；
        或 0 个 Red Flag 但盈利质量不够优秀（净利率 ≤5% 且 机构有卖出）
   → 降级为 定投Buy▲，常规 1.0 倍额度，不加倍，文案标注「存疑，待拐点确认」
   ⚠️ 0 个 Red Flag 绝不可判为"基本面恶化型"——没有硬伤只是不够优秀，
      典型如 TSLA（净利率 3.95%、机构卖出 9 家、但盈利未崩、无亏损）应落此档

🔴 基本面恶化型（大幅降级）
   条件：命中 ≥2 个 Red Flag
   → 降级为 观望— 或 0.3~0.5 倍试探额度，文案必须标注「基本面恶化型，技术超卖救不了盈利崩塌」
   → 严禁写"加倍抄底"「逢跌加仓」等鼓励性文案
```

**输出要求**：
1. 逐股详解的 `🎯 定投信号` 条目必须写成 `<技术档位> → <校验后可执行档位>（<分型>）— <量化依据>`
   例：`Strong Buy▲▲ → 观望（🔴基本面恶化型）— 回撤-46.8%+RSI6 24.8命中规则，但净利同比-68%、净利率0.5%，命中2项Red Flag`
2. 汇总表新增一列 **`基本面校验`**，取值 `🟢技术错杀 / 🟡存疑 / 🔴基本面恶化 / —（未触发校验）`
3. 微信简报的定投信号行只统计**校验后**的档位，避免夸大买入信号数量
4. 若同时出现多只 Strong Buy，必须在「建议动作」里给出**优先级排序**（按 Red Flag 数量升序 + 净利率降序），而不是并列罗列

**🚨 铁律**：报告中出现的每一个 `Strong Buy▲▲`，都必须能回答"它的基本面为什么没坏"。答不出来就降级。宁可少推荐，不可误导加倍抄底。
...
3. **总览图（render_overview.py 生成的 PNG）新增"定投"列**：脚本内置 `advice_key()` + `ADV` 五档表，优先读股票 `advice_key`/`advice_label` 字段，无则按上述规则现算。因 matplotlib 字体不支持 emoji 圆圈，图内用文字符号「Strong Buy▲▲(深红#b91c1c) / 定投Buy▲(红#dc2626) / 观望—(灰#6b7280) / 定投Sell▼(绿#16a34a) / Strong Sell▼▼(深绿#15803d)」。构造 overview_data 时带上 `rsi6` 字段可保证图与 HTML 口径一致
   - 画布布局（2026-07-28 调优）：W=1090，8 列 x 坐标 `[92, 196, 305, 415, 515, 618, 738, 900]`。代码列从 70→92 避免贴住左侧色条；信号列文案精简后整体收窄，消除右侧过量留白
   - 🎨 **双色系语义隔离铁律（2026-07-28 确立，勿回归）**：图中只允许两套互不干扰的色系。①**红/绿专属涨跌与买卖方向**（涨/买=红，跌/卖=绿）②**左侧异常等级条 + 信号列文字统一用中性色阶**：无异常=浅灰 `#cbd5e1`（信号文字用 `#8894a8`）/ 一般异常=琥珀 `#f59e0b` / 重大波动>4%=深蓝 `#1d4ed8`。**严禁**异常等级条或"正常"文字使用红/绿——曾因绿色"正常"被误读为下跌、红色异常条被误读为上涨
   - ✂️ **信号列文案必须精简**：去掉括号内百分比（`日涨跌>4%(-5.17%)` → `波动>4%`，百分比在"今日涨跌"列已有，重复占宽）；缩写映射 `日涨跌>4%→波动>4%`、`接近52W新高→近52W高`、`量比>2→放量`；多信号时用 `+N` 表示剩余条数；超 11 字符截断

**微信简报补充**：第二条 notify 的"📈 自选股行情"区块新增一行 `・定投信号：` 汇总当日出现的买/卖信号（如 `Strong Buy 1 只(MU) / Buy 3 只(XPEV/CRCL/NOK) / Sell 1 只(INTC) / 其余观望`），无买卖信号时写"全部观望"。
- ⚠️ **必须统计"二次基本面校验后"的档位**，不能用机械规则的原始档位（否则会夸大 Strong Buy 数量误导决策）
- 若有标的被降级，需额外加一行 `・校验降级：<SYMBOL> Strong Buy→观望（净利同比-68%）`，让用户知道哪些技术信号被基本面否决

**颜色纪律**：买入侧=红系（Strong Buy `#b91c1c` 深红 / Buy `#dc2626` 红，与红涨绿跌一致，买入=看多=红），卖出侧=绿系（Strong Sell `#15803d` 深绿 / Sell `#16a34a` 绿），观望=灰 `#6b7280`。
...
- message 格式（严格按模版，**首行仅保留站点根地址**）：
  ...
  ━━━ 📈 自选股行情 ━━━
  ・异常信号：<列 1-3 条关键异常>
  ・定投信号：<Strong Buy X 只(代码) / Buy Y 只(代码) / Sell Z 只(代码) / 其余观望；无买卖则写"全部观望">
  ・建议动作：<列 1-2 条>
